...
bool generated_font_texture_id = false;
...
void Utility::draw_text(ShaderProgram* program, std::string text, float screen_size, float spacing, glm::vec3 position)
{
    ...
    // i did this to simplifiy the draw_text function without needing to run load_texture every frame for performance
    while (!generated_font_texture_id) {
        font_texture_id = Utility::load_texture(FONTSHEET_FILEPATH);
        generated_font_texture_id = true;
    }
